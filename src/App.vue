        ...
        <section class="panel modal-card settings-modal" role="dialog" aria-modal="true" aria-labelledby="settings-title">
          ...
          <div class="modal-body settings-body">
            <section class="settings-section">
              ...
              <label class="field">
                <span>K12 Workspace ID（一行一个或逗号分隔；每个邮箱会逐个加入所有未屏蔽的空间）</span>
                <textarea v-model="workspaceText" class="workspace-box"></textarea>
              </label>
              <div class="blocked-workspaces">
                <div class="blocked-head">
                  <strong>已屏蔽的 workspace（加入失败自动屏蔽，不再加入）</strong>
                  <span class="pill">{{ form.blockedWorkspaceIds.length }}</span>
                </div>
                <ul v-if="form.blockedWorkspaceIds.length" class="blocked-list">
                  <li v-for="id in form.blockedWorkspaceIds" :key="id">
                    <span class="mono clipped">{{ id }}</span>
                    <button class="ghost tiny" :disabled="unblockingWorkspaceId === id" @click="unblockWorkspace(id)">
                      {{ unblockingWorkspaceId === id ? "解除中..." : "解除屏蔽" }}
                    </button>
                  </li>
                </ul>
                <p v-else class="hint">暂无被屏蔽的 workspace。加入失败的空间会自动进入此名单。</p>
              </div>
              <div class="switch-grid">
                <label class="switch-card">
                  ...
                  <span>
                    ...
                    <small>每个邮箱任务会逐个加入所有未屏蔽的 workspace 执行 request/accept；加入失败的空间会被永久屏蔽。</small>
                  </span>
                </label>
                <label class="switch-card">
                ...
                <label class="switch-card">
                  ...
                  <span>
                    ...
                    <small>开启后跳过 Sub2API OAuth：注册/登录 → 逐个加入并切到每个 K12 空间 → 每个空间用各自的 K12 AT 分别入库一份 --noRT 账号。</small>
                  </span>
                </label>
              </div>
...
const unblockingWorkspaceId = ref("");
...
async function loadConfig() {
  ...
  Object.assign(form, {
    ...
    blockedWorkspaceIds: config.blockedWorkspaceIds || [],
...
async function saveConfig() {
  ...
  try {
    ...
    // blockedWorkspaceIds 由后端自动维护（加入失败即屏蔽），这里不回传，避免覆盖运行中任务刚写入的屏蔽项。
    const {blockedWorkspaceIds: _ignoredBlocked, ...formPayload} = form;
    const payload = {
      ...formPayload,
...
async function unblockWorkspace(id: string) {
  if (unblockingWorkspaceId.value) return;
  unblockingWorkspaceId.value = id;
  try {
    const next = form.blockedWorkspaceIds.filter((item) => item !== id);
    await api<any>("/api/config", {method: "PATCH", body: JSON.stringify({blockedWorkspaceIds: next})});
    form.blockedWorkspaceIds = next;
    showToast("已解除屏蔽，该 workspace 后续会重新加入");
  } catch (error) {
    showToast(`解除屏蔽失败：${error instanceof Error ? error.message : String(error)}`);
  } finally {
    unblockingWorkspaceId.value = "";
  }
}
